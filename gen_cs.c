						   ...
						   TreeCCNode *node)
{
	...
	if(node->parent)
	{
		...
		if(isAbstract)
			TreeCCStreamPrint(stream, "%s partial abstract class %s : %s\n{\n", accessMode, node->name, node->parent->name);
		else
			TreeCCStreamPrint(stream, "%s partial class %s : %s\n{\n", accessMode, node->name, node->parent->name);
	}
	else
	{
		...
		if(isAbstract)
		{
			if(context->baseType)
				...
				TreeCCStreamPrint(stream, "%s partial abstract class %s\n{\n", accessMode, node->name);
		}
		else
		{
			if(context->baseType)
				...
				TreeCCStreamPrint(stream, "%s partial class %s\n{\n", accessMode, node->name);
		}
		...
		if(context->track_lines)
		{
			TreeCCStreamPrint(stream, "\tpublic String getFilename() { return filename__; }\n");
			TreeCCStreamPrint(stream, "\tpublic long getLinenum() { return linenum__; }\n");
			TreeCCStreamPrint(stream, "\tpublic void setFilename(String filename) "
					"{ filename__ = filename; }\n");
			TreeCCStreamPrint(stream, "\tpublic void setLinenum(long linenum) "
					"{ linenum__ = linenum; }\n");
		}
		TreeCCStreamPrint(stream, "\n");
	}
...
static void ImplementStateType(TreeCCContext *context, TreeCCStream *stream)
{
	...
	if(!(context->reentrant))
	{
		TreeCCStreamPrint(stream, "\tprotected static %s state__;\n\n",
						  context->state_type);
...
static void WriteCSharpHeaders(TreeCCContext *context)
{
	...
	while(stream != 0)
	{
		if(!(stream->isHeader))
		{
			...
			if (context->using) {
				int n = 0;
				while (context->using[n]) {
					TreeCCStreamPrint(stream, "using %s;\n", context->using[n]);
					n += 1;
				}
			}
