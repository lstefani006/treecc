...
/*
 * "using": specify the namespace to use for subsequent definitions.
 */
static int UsingOption(TreeCCContext *context, char *value, int flag)
{
	if(!value)
	{
		return TREECC_OPT_NEED_VALUE;
	...
	else
	{
		...
		context->using = realloc(context->using, (n + 2) * sizeof(char *));
		context->using[n] = value;
		context->using[n + 1] = 0;
		return TREECC_OPT_KEEP_VALUE;
	}
}
